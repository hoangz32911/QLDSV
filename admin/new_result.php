	...
	<div class="card card-outline card-primary">
				...
				<div class="row justify-content-center">
					<div class="col-md-6">
						...
						<div class="form-group">
							<label for="" class="control-label">Thông tin sinh viên:</label>
							...
							<small id="class"><?php echo isset($class) ? "Thông tin lớp học:  " . $class : "" ?></small>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="d-flex justify-content-center align-items-center">
							...
							<div class="form-group col-sm-3">
								<label for="" class="control-label">Điểm:</label>
								<input type="text" class="form-control form-control-sm text-center" id="mark" maxlength="6">
							</div>
							<button class="btn btn-sm btn-primary bg-gradient-primary" type="button" id="add_mark">Thêm</button>
						</div>
					</div>
					<hr>
					<div class="col-md-8 offset-md-2">
						<table class="table table-bordered" id="mark-list">
								...
								<?php if (isset($id)) : ?>
									...
									?>
										<tr data-id="<?php echo $row['sid'] ?>">
											<td class="text-center"><input type="hidden" name="subject_id[]" value="<?php echo $row['subject_id'] ?>"><?php echo $row['subject_code'] ?></td>
							...
							<tfoot>
								<tr>
									<th colspan="2" class="text-center">Điểm TB tổng các môn</th>
		...
		<div class="card-footer border-top border-info">
			<div class="d-flex w-100 justify-content-center align-items-center">
				<button class="btn btn-flat  bg-gradient-primary mx-2" form="manage-result">Lưu</button>
				<a class="btn btn-flat bg-gradient-secondary mx-2" href="./index.php?page=results">Thoát</a>
			</div>
		</div>
	</div>
</div>
<script>
	$('#student_id').change(function() {
		...
		$('#class').text("Thông tin lớp học: " + _class);
	})
	$('#add_mark').click(function() {
		var subject_id = $('#subject_id').val()
		var mark = $('#mark').val()
		if (subject_id == '' && mark == '') {
			alert_toast("Vui lòng chọn môn học và nhập dấu trước khi thêm vào danh sách.", "error");
			return false;
		}
		var sData = $('#subject_id option[value="' + subject_id + '"]').attr('data-json')
		sData = JSON.parse(sData)
		if ($('#mark-list tr[data-id="' + subject_id + '"]').length > 0) {
			alert_toast("Môn học đã có trong danh sách.", "error");
			return false;
		}
	...
	function calc_ave() {
		...
		$('#average').text(parseFloat(total / i).toLocaleString('en-US', {
			style: 'decimal',
			maximumFractionDigits: 2
		}))
		$('[name="marks_percentage"]').val(parseFloat(total / i).toLocaleString('en-US', {
			style: 'decimal',
			maximumFractionDigits: 2
		}))
	}
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					...
					$('#msg').html('<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Mã sinh viên không tồn tại.</div>')
					end_load()
				}
			}
		})
