...
class Action
{
	...
	function save_khoa()
	{
		...
		$chk = $this->db->query("SELECT * FROM khoa where ma_khoa ='$ma_khoa' and ten_khoa = '$ten_khoa' and id != '$id' ");
		if ($chk->num_rows > 0) {
			return 2;
			exit;
		}
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO khoa set $data");
		} else {
			$save = $this->db->query("UPDATE khoa set $data where id = $id");
		}
		if ($save) {
			return 1;
		}
	}
	function delete_khoa()
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM khoa where id = $id");
		if ($delete) {
			return 1;
		}
	}
	function save_giang_vien()
	{
		extract($_POST);
		$data = "";
		foreach ($_POST as $k => $v) {
		...
		$chk = $this->db->query("SELECT * FROM giang_vien where ma_gv ='$ma_gv' and sdt = '$sdt' and id != '$id' ");
		if ($chk->num_rows > 0) { 
			return 2;
			exit;
		}
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO giang_vien set $data");
		} else {
			$save = $this->db->query("UPDATE giang_vien set $data where id = $id");
		}
		if ($save) {
			return 1;
		}
	}
	function delete_giang_vien()
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM giang_vien where id = $id");
		if ($delete) {
			return 1;
		}
	}
